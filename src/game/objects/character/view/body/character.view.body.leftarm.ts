...
export default class character_view_body_leftarm extends Phaser.GameObjects.Container {
    private _upper_part_left_arm: Phaser.GameObjects.Container;
    private _lower_part_left_arm: Phaser.GameObjects.Container;
    ...
    private create(){
        this._upper_part_left_arm = this.scene.add.container(0, 0);
        const upperImg = this.scene.add.image(0, 0, "arm_part")
            .setOrigin(0.5, 0);

        this._upper_part_left_arm.add(upperImg);
    
        this._upper_part_left_arm.angle = 30;
    
        this._lower_part_left_arm = this.scene.add.container(0, upperImg.height - 10);
    
        const lowerImg = this.scene.add.image(0, 0, "arm")
            .setOrigin(0.5, 0);
        this._lower_part_left_arm.add(lowerImg);
        this._lower_part_left_arm.angle = -30;
    
        this._upper_part_left_arm.add(this._lower_part_left_arm);
        this.add(this._upper_part_left_arm);
    }

    public idleAnimation(){
        const tweenUpper = this.scene.tweens.add({
            targets: [this._upper_part_left_arm],
            angle: {from: 30, to: -5},
            duration: 1000,
        ...
        const tweenLower = this.scene.tweens.add({
            targets: [this._lower_part_left_arm],
            angle: {from: -30, to: -35},
            duration: 1000,
            ease: Ease.EaseSine_easeInOut,
            yoyo: true,
    ...
    public failAnimation(durationPhase1: number, delayPhase2: number = durationPhase1, durationPhase2: number = durationPhase1){
        const tweenUpper = this.scene.tweens.add({
            targets: this._upper_part_left_arm,
            angle: {from: this._upper_part_left_arm.angle, to: -80},
        ...
        const tweenLower = this.scene.tweens.add({
            targets: this._lower_part_left_arm,
            angle: {from: this._lower_part_left_arm.angle, to: 0},
            duration: durationPhase1,
            ease: Ease.EaseSine_easeInOut,
            yoyo: false,
            repeat: 0,
    ...
    private backToIdleAnimation(duration: number){
        const tweenUpper = this.scene.tweens.add({
            targets: this._upper_part_left_arm,
            angle: {from: this._upper_part_left_arm.angle, to: 30},
                    duration: duration,
                    ease: Ease.EaseSine_easeInOut,
                    yoyo: false,
                    repeat: 0,
            ...
            const tweenLower = this.scene.tweens.add({
                    targets: this._lower_part_left_arm,
                    angle: {from: this._lower_part_left_arm.angle, to: -30},
                    duration: duration,
                    ease: Ease.EaseSine_easeInOut,
                    yoyo: false,
                    repeat: 0,
    ...
    public successAnimation(durationPhase1: number, delayPhase2: number = durationPhase1, durationPhase2: number = durationPhase1){
        const tweenUpper = this.scene.tweens.add({
            targets: this._upper_part_left_arm,
            angle: {from: this._upper_part_left_arm.angle, to: 90},
            duration: durationPhase1,
            ease: Ease.EaseSine_easeInOut,
            yoyo: false,
            repeat: 0,
        ...
        const tweenLower = this.scene.tweens.add({
            targets: this._lower_part_left_arm,
            angle: {from: this._lower_part_left_arm.angle, to: 90},
            duration: durationPhase1,
            ease: Ease.EaseSine_easeInOut,
            yoyo: false,
            repeat: 0,
