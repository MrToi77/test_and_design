...
export default class character_view_body_leftleg extends Phaser.GameObjects.Container {
    private _upper_part_left_leg: Phaser.GameObjects.Container;
    private _lower_part_left_leg: Phaser.GameObjects.Container;
    ...
    private create(){
        const legPartHeight = this.scene.textures.get("leg_part").getSourceImage().height;
        this._upper_part_left_leg = this.scene.add.container(0, legPartHeight);
        const upperImg = this.scene.add.image(0, 0, "leg_part").setOrigin(0.5, 1);
        this._upper_part_left_leg.add(upperImg);
        this._lower_part_left_leg = this.scene.add.container(0, upperImg.height - 10);
        const lowerImg = this.scene.add.image(0, 0, "leg").setOrigin(0.5, 0);
        this._lower_part_left_leg.add(lowerImg);
        this.add(this._lower_part_left_leg);
        this.add(this._upper_part_left_leg);
    }

    public idleAnimation(){
        this.angle = 15;
        const tweenUpper = this.scene.tweens.add({
            targets: this._upper_part_left_leg,
            angle: {from: -3, to: -15},
    ...
    public stopAnimation(){
        ...
        this._upper_part_left_leg.angle = 0;
        this._lower_part_left_leg.angle = 0;
        this.angle = 0;
    }
    ...
    public successAnimation(durationPhase1: number, delayPhase2: number = durationPhase1, durationPhase2: number = durationPhase1){
        ...
        const tweenLower = this.scene.tweens.add({
            targets: this._lower_part_left_leg,
            angle: {from: this.angle, to: -50},
            duration: durationPhase1,
            ease: Ease.EaseSine_easeInOut,
            yoyo: false,
            repeat: 0,
        ...
        this.scene.time.delayedCall(delayPhase2, () => {
            this.scene.tweens.add({
                    targets: this._lower_part_left_leg,
                    angle: {from: this._lower_part_left_leg.angle, to: 0},
